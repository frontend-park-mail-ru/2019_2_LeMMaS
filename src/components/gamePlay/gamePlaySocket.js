...
export default class GamePlay {
    ...
    start = () => {
        ...
        this.socket.onmessage = (event) =>  {
            ...
            if(data.body.body) {
                this.easingTargetX = data.body.body.x;
                this.easingTargetY = data.body.body.y;
                //this._moveMyBall();
            } else {
                this.easingTargetX = data.body.player_position.x;
                this.easingTargetY = data.body.player_position.y;
                //this._moveMyBall();
            }
        };
    ...
    _handleMouseMove = event => {
        const dis = Math.sqrt( Math.pow(event.clientX - this.ball.x, 2) + Math.pow(event.clientY - this.ball.y,2) );

        let angle = Math.round(Math.acos((event.clientY - this.ball.y) / dis) / Math.PI * 180);

        if ((event.clientX - this.ball.x > 0 && event.clientY - this.ball.y < 0) || (event.clientX - this.ball.x > 0 && event.clientY - this.ball.y > 0)) {
            angle = 180 + (180 - angle);
        }
        ...
        this.socket.send(`{"type":"direction", "direction":${angle}}`);
