...
export default class MultiPlayer {
                    ...
                    data.players.forEach(player => {
                        ...
                        API.getAvatarById(player.user_id).then(user => {
                            if (user.avatar_path) {
                                const backgroundImage = new Image();
                                backgroundImage.src = user.avatar_path;
                                const currentBall = this.balls.get(
                                    player.user_id
                                );
                                if (currentBall) {
    ...
    _countAndSendDirection = (x, y) => {
        ...
        let angle =
            ...
            Math.round(
                (Math.acos(
                    (event.clientY - this.balls.get(this.currentUserID).y) / dis
                ) /
                    Math.PI) *
                    180
            );
        ...
        if (
            (event.clientX - this.balls.get(this.currentUserID).x > 0 &&
                event.clientY - this.balls.get(this.currentUserID).y < 0) ||
            (event.clientX - this.balls.get(this.currentUserID).x > 0 &&
                event.clientY - this.balls.get(this.currentUserID).y > 0)
        ) {
            angle = 180 + (180 - angle);
