...
export default class API {
    static registerUser(email, name, password) {
        return httpNetwork.post([BACKEND_URL, API_V1_PATH_PREFIX, routes.USER_REGISTER_PATH].join("/"), {
            email,
            name,
            password,
        });
    ...
    static changeUserData(name, password) {
        return httpNetwork.post([BACKEND_URL, API_V1_PATH_PREFIX, routes.USER_SETTINGS_PATH].join("/"), {
            name,
            password,
        });
    }
    ...
    static changeAvatar(formData) {
        return httpNetwork.post([BACKEND_URL, API_V1_PATH_PREFIX, routes.USER_AVATAR_UPLOAD_PATH].join("/"), formData);
    }

    static getAvatarPreviewUrl(name) {
        const path = [BACKEND_URL, API_V1_PATH_PREFIX, routes.USER_AVATAR_PREVIEW_PATH].join("/") + "?name=" + name;
        return httpNetwork.get(path)
            .then(response => response.json())
            .then(response => response.body.avatar_url);
    }

    static currentUserProfile() {
        return httpNetwork.get([BACKEND_URL, API_V1_PATH_PREFIX, routes.CURRENT_USER_PROFILE_PATH].join("/"))
            .then(response => response.json())
            .then(response => response.body.user);
    }

    static listUsers() {
        return httpNetwork.get([BACKEND_URL, API_V1_PATH_PREFIX, routes.USER_LIST_PATH].join("/"))
            .then(response => response.json())
